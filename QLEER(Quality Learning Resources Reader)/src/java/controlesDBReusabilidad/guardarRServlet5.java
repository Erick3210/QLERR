 ...
 * @author efcas
 */
@WebServlet(name = "guardarRServlet5", urlPatterns = {"/guardarRServlet5"})
public class guardarRServlet5 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            guardarArchivos gDOCX = new guardarArchivos();
            ...
            gc.guardar(comT);
//            gd.guardar(disponibilidad);
            gDC.guardar(escolaridad, dominio, tema, tipodecontenido, service);

           
//            double dis = oD.ObtenerDatosDisponibilidad1();
//            double usa = oU.ObtenerDatosUsabilidad1();
//            double suma = comT + dis + usa;
//            double reusabilidadfinal = (suma * 100) / 4;

//            gr.guardar(reusabilidadfinal, formato);
            ...
            while (true) {
                try {
                    Class.forName("generated_sources.jax_ws.swadocx." + ServicioRecortado + "_Service");
                    break;
                    ...
                    if (contador >= 80) { // 180 segundos = 3 minutos
                        System.out.println("Tiempo de espera agotado. Clases proxy generadas y reconocidas.");
                        break;
                    }
                }
            }
            ...
            try {
                ...
                Class<?> claseServicio = Class.forName("swadocx." + ServicioRecortado + "_Service");
                ...
                String path = "C:\\Users\\efcas\\OneDrive\\ESCRITORIO 2\\NetBeansProjects\\Login1\\QLEER(Quality Learning Resources Reader)\\web\\recursosDOCX\\";
...
// verificar si ya existe un archivo con el mismo nombre y versión
                ...
                gDOCX.guardarDOCX(filename + "_v" + version + extension);
// crear un nuevo archivo con el nombre y versión adecuados
                ...
                String direccionDOCX = path + filename + "_v" + version + extension;

                List<String> lista1 = ObtenerAlineacionDocX(direccionDOCX);

                int totalParrafos = lista1.size();
                int ParrafosAdentro = 0; //LEFT
                int ParrafosAfuera = 0;

                for (String elemento : lista1) {

//                        out.println(elemento); 
                    if (elemento.equals("CENTER")) {
                        ParrafosAfuera++;
                    }
                    if (elemento.equals("LEFT")) {
                        ParrafosAdentro++;
                    }
                    if (elemento.equals("RIGHT")) {
                        ParrafosAdentro++;
                    }

                    if (elemento.equals("BOTH")) {
                        ParrafosAfuera++;
                    }

                }

//                    a.guardar(totalParrafos, ParrafosAdentro, ParrafosAfuera);
                double porcentajeAlien = (double) ParrafosAdentro / (double) totalParrafos;
                a.guardar(totalParrafos, ParrafosAdentro, ParrafosAfuera, porcentajeAlien);

                List<String> lista2 = TipoDeLetraDocX(direccionDOCX);

                int totalPalabras = lista2.size();
                int Arial = 0;
                int TimesNewRoman = 0;
                int Calibri = 0;
...
//                        out.println(elemento); 
                    ...
                    if (elemento.equals("Times New Roman")) {
                        TimesNewRoman++;
                    }
                ...
                OtrasFuentes = totalPalabras - (Arial + TimesNewRoman + Calibri);

//                    t.guardar(totalPalabras, Arial, TimesNewRoman, OtrasFuentes, Calibri);
                double porcentajeTLetra = ((double) Arial + (double) TimesNewRoman + (double) Calibri) / (double) totalPalabras;
                t.guardar(totalPalabras, Arial, TimesNewRoman, OtrasFuentes, Calibri, porcentajeTLetra);

                List<String> lista3 = ColorDeLaFuenteDocX(direccionDOCX);

                int totalPalabras2 = lista3.size();
                int Negro = 0;
                int OtrosColores = 0;
...
//                    Cfuente.guardar(totalPalabras2, Negro, OtrosColores);
                double porcentajeColorLetra = (double) Negro / (double) totalPalabras2;
                Cfuente.guardar(totalPalabras2, Negro, OtrosColores, porcentajeColorLetra);

                List<String> lista4 = TamañoDeLaFuenteDocX(direccionDOCX);

                int totalPalabras3 = lista4.size();
                int Rango1 = 0;
                int Rango2 = 0;
                int Rango3 = 0;
                ...
                Rango3 = totalPalabras3 - (Rango1 + Rango2);

//                    ta.guardar(totalPalabras3, Rango1, Rango2, Rango3);
                double porcentajeTmaño1 = ((double) Rango1 + (double) Rango2) / (double) totalPalabras3;
                ta.guardar(totalPalabras3, Rango1, Rango2, Rango3, porcentajeTmaño1);
...
//                        out.println(elemento); 
                ...
                double porcentajeCfondo1 = 0;
                out.println("COLOR DE FONDO FUE: " + colorF);

                if (colorF.equals("(255,255,255)")) {
                ...
                Cfont.guardar(colorF, porcentajeCfondo1);
...
//                double legibilidad1 = (porcentajeAlien + porcentajeTLetra + porcentajeColorLetra + porcentajeTmaño1 + porcentajeCfondo1) / 5;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(guardarRServlet5.class.getName()).log(Level.SEVERE, null, ex);
